const domString = (movieArray, config, whereToPrint, myCollectionMode = false) => {
  ...
  movieArray.forEach((movie, index) => {
    ...
    // print only if it comes from database
    if (myCollectionMode) {
      strang += `<a class="btn deleteMovieFromCollectionEvent">X</a>`;
    }
    strang +=     `<img data-poster="${movie.poster_path}" class="movie-poster_path" src="${config.base_url}/w342/${movie.poster_path}" alt="...">`;
    ...
    if (!myCollectionMode) {
      ...
      strang += `<p>I am going to put star rating here one day.</p>`;
    }
    strang +=     `</div>`;
    strang +=   `</div>`;
    strang += `</div>`;
